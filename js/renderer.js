...
/**
 * Helper function to get the system prompt array from an entry, handling both cached and inline.
 * @param {object} entry
 * @returns {Array}
 */
...
export function renderRequestTab(entry, callbacks = {}) {
  ...
  if (entry.anthropicRequest) {
    ...
    const systemData = getSystemPrompt(entry);
    summary.system = systemData.map((s, i) => {
      const text = s.text || s.content || JSON.stringify(s);
